...
rule nucleotide_table_overrep:
    input:
        rules.bed2fasta_xr_filtered.output,
    output:
        dinuc="results/{method}/{samples}/{samples}_{build}_filtered_sorted_dinucleotideTable.txt",
        nuc="results/{method}/{samples}/{samples}_{build}_filtered_sorted_nucleotideTable.txt",
    log:
        "logs/rule/nucleotide_table_overrep/{samples}_{build}_{method}.log",
    benchmark:
        "logs/rule/nucleotide_table_overrep/{samples}_{build}_{method}.benchmark.txt",
    shell:  
        """
        (echo "`date -R`: Calculating dinucleotide abundance table..." &&
        workflow/scripts/fa2kmerAbundanceTable.py \
...
rule plot_nuc_overrep:
    input:
        dinuc=rules.nucleotide_table_overrep.output.dinuc,
        nuc=rules.nucleotide_table_overrep.output.nuc,
    output:
        dinuc=report("results/processed_files/{samples}_{build}_{method}_filtered_dinucleotideTable.pdf", 
                    category="Nucleotide Content"),
        nuc=report("results/processed_files/{samples}_{build}_{method}_filtered_nucleotideTable.pdf", 
                    category="Nucleotide Content"),
    params:
        motif=lambda w: getDinuc(w.samples, config["meta"][w.samples]["product"]),
        name="{samples}_{build}_{method}_filtered",
    log:
        "logs/rule/plot_nuc_overrep/{samples}_{build}_{method}.log",
    benchmark:
