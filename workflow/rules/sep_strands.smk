...
rule sep_strands_sim:
    ...
    log:
        "logs/rule/analysis/{samples}/{samples}_{build}_{method}_sep_strands_sim.log",
    benchmark:
        "logs/rule/analysis/{samples}/{samples}_{build}_{method}_sep_strands_sim.benchmark.txt",
    shell:  
        """
        (echo "`date -R`: Separating plus stranded reads..." &&
        awk '{{if($6=="+"){{print}}}}' {input} > {output.plus} &&
