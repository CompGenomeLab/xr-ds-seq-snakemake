...
p <- add_argument(
  p, "-k",
  help = paste(
    "kmer value of input, 1 (nucleotide) or 2 (dinucleotide)"
  )
)
p <- add_argument(
  p, "-s",
...
p <- add_argument(
  p, "-f",
  help = paste(
    "filter (only for dinucleotides). Ex: 'CC','CT','TC','TT'"
  )
)
...
if (argv$k == "1") {
  for (i in 2:ncol(nuc_table)) {
    colnames(nuc_table)[i] <- paste(i - 1)
  ...
  for (i in 2:ncol(nuc_table)) {
    colnames(nuc_table)[i] <- paste(i - 1, "-", i, sep = "")
...
log_info("Ordering columns...")
dt_organized$positions <- factor(
  dt_organized$positions,
  levels = x_order
)
...
log_info("Plotting...")
p <- ggplot(dt_organized, aes(
  x = positions, y = freq, fill = nucleotides
)) +
  geom_bar(stat = "identity") +
  ylim(0, 101) +
  ylab("Frequency (%)") +
  xlab("Position in oligomers") +
  labs(
    title = mytitle,
